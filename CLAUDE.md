...
- `server/index.js` is Express + Socket.io. Every event replies via ack with `{ ok, error?, ...data }`. After any change, `broadcast(room)` pushes a fresh `state` to the host room (`CODE:host`) and to each player's room (`CODE:p:<id>`). Events: `host:create`, `host:resume`, `player:join`, `player:resume`, `player:leave`, `room:kick`, `room:addBots`, `game:start`, `game:reset`, `game:action`. The host screen or the first *human* player to join ("VIP") can start and reset the game.
- **Test mode:** `/host?test` shows bot controls (hidden, not secured; bots only affect that room). Bots are players with `bot: true` and no token; `scheduleBots(room)` in `server/index.js` runs after every broadcast and makes one bot move at a time on a timer (`BOT_DELAY_MS`), using `botAction(view, id)` from `server/bots.js`. `server/bots.test.js` checks bots alone can always finish a game.
